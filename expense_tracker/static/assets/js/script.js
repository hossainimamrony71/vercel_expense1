$(document).ready(function() {
    ...
    if ($('.datanew').length > 0) {
        $('.datanew').DataTable({
            ...
            "language": {
                ...
                emptyTable: "No data available in table" // Add empty table message
            },
            initComplete: function(settings, json) {
                var $filter = $('.dataTables_filter');
                // Append to elements only if they exist
                if ($('#tableSearch').length) {
                    $filter.clone().appendTo('#tableSearch');
                }
  ...
  // Initially load TED data
  var currentLabels = tedLabels;
  var currentValues = tedValues;

  // Check if pieChart already exists
  var pieChart;
  function initializeUniqueChart() {
    ...
    pieChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          legend: {
            display: true,
            position: 'left', // Moves labels to the right side
            labels: {
              boxWidth: 15,
              padding: 10
            }
  ...
  function updateUniqueChart(newLabels, newData) {
  ...
  if (!pieChart) {
    initializeUniqueChart();
  }

  // Toggle functionality for TED and S2L using the list items
  document.querySelectorAll('#dataToggleUnique li').forEach(function(item) {
    item.addEventListener('click', function() {
      // Remove active class from all toggle items
      document.querySelectorAll('#dataToggleUnique li').forEach(function(el) {
        el.classList.remove('active');
      });
      // Add active class to the clicked item
      this.classList.add('active');

      // Update chart based on selected data type
      var type = this.getAttribute('data-type');
      if (type === 'ted') {
        updateUniqueChart(tedLabels, tedValues);
      } else if (type === 's2l') {
        updateUniqueChart(s2lLabels, s2lValues);
      }
    });
  });